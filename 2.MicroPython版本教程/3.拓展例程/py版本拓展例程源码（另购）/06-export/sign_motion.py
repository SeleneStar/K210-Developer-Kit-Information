...
def car_control(class_id):
    global car_count, car_state
    car_state = 1
    if class_id == 1:# 向左转 turn left
        car_state = 1
        car_count = 10
        bot.set_car_motion(0, 0, 3)
    elif class_id == 2:# 向右转 turn right
        car_state = 1
        car_count = 10
        bot.set_car_motion(0, 0, -3)
    elif class_id ==3: # 停止 stop
        car_state = 0
        car_count = 0
        bot.set_car_motion(0, 0, 0)
...
# main loop
features = []
THRESHOLD = 99.5    # 比对阈值，越大越严格 Alignment threshold, the larger the more stringent
