...
enum errorcode_e wav_init( wav_file_t *wav_file)
{
	...
	{
		printf("wav_init: f_lseek failed!\r\n");
		return FILE_FAIL;
	}
...
enum errorcode_e wav_decode_init( wav_file_t *wav_file)
{
	wav_file->buff_end = 0;
	wav_file->buff0_len = 256 * 1024;
	wav_file->buff1_len = 256 * 1024;
	wav_file->buff0  = (uint8_t *)iomem_malloc(wav_file->buff0_len);
	wav_file->buff1  = (uint8_t *)iomem_malloc(wav_file->buff1_len);

	if(wav_file->buff0 == NULL | wav_file->buff1 == NULL)
	{
		printf("buff0 buff1 iomem_malloc failed, %d, %d\r\n", wav_file->buff0_len, wav_file->buff1_len);
		return FILE_FAIL;
	}
	
	//printf("add1:%x, flag:%d\r\n", (int)wav_file->fp, wav_file->fp->flag);
...
enum errorcode_e wav_decode( wav_file_t *wav_file)
{
	if (wav_file->buff0_used == 0) 
	{
		...
		if (FR_OK != f_read(wav_file->fp, wav_file->buff0, wav_file->buff0_len, &(wav_file->buff0_read_len)))
		{
			printf("wav_decode: f_read1 failed!\r\n");
			return FILE_FAIL;
		}
	...
	else if (wav_file->buff1_used == 0) 
	{
		...
		{
			printf("wav_decode: f_read2 failed!\r\n");
			return FILE_FAIL;
		}
