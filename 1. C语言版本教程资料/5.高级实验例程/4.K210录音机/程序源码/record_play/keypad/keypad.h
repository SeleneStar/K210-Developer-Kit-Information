...
#define KEY_TICKS         5              // 按键扫描周期（ms）,keyscan()函数在哪个固定扫描周期中 该值就等于多少 原则上应该是10的公约数中的值 因为按键消抖是10ms
#define RELEASE           0              // 按键松开
#define PRESS             1              // 按键按下
#define KEY_STATUS        4              // 按键事件，松开、短按、长按、重复

#define KEY_FILTER_TIME   10             // 按键消抖时间
#define KEY_LONG_TIME     1500           // 长按触发时间(ms)=KEY_LONG_TIME/KEY_TICKS
#define KEY_REPEAT_TIME   200            // 连发间隔,连发个数(/s)=1000*KEY_TICKS/KEY_REPEAT_TIME
